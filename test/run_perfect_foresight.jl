...
## model creation
SampleScenario  = SamplePath(TransProb, NSamples);
...
for i = 1:NSamples
    ...
    for sn=1:NSubnetworks[2], n = LVBatteryNodes, t = 1:H
        fix(PFProblem.balance_RHS[sn,n,t],  LVDemandDataDict[n][t] - (LVPVNodeDict[n] * NormPV[t][SampleScenario[t,i]]));
    end
...
p_in_average, p_out_average = ComputePinPoutAverage(SampleScenario,p_out_Store,NSamples)
## save as jld2 file
